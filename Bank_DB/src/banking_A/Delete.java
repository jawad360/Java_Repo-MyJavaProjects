	...
	public Delete(final String tableN) throws SQLException {
		...
		deleteBt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) 
			{
				...
				{
					if(tableN.equals("CUSTOMERS"))
						st.executeQuery("delete from CUST_1A where "+title[0]+" = "+drivers[delNo][0]);
					else if(tableN.equals("ACCOUNT"))
					{
						st.executeQuery("delete from ACCSAV_1A where "+title[0]+" = "+drivers[delNo][0]);
						st.executeQuery("delete from ACCCUR_2A where "+title[0]+" = "+drivers[delNo][0]);
					}
					else
